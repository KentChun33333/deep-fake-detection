...
class PairClasssifer_DeepFaceMultiSys(pl.LightningModule):
    ...
    def sys_forward(self, i):
        pre_imgs = i['frames'].permute(0, 1, 2, 5, 3, 4).float()

        y_hat = self.model( pre_imgs )
...
class PairClasssifer_DeepFaceSys(pl.LightningModule):

    def __init__(self, hparams):
        super(PairClasssifer_DeepFaceSys, self).__init__()
        # train, test, train_labels, 
        self.hparams = hparams
    ...
    def build_dataset(self, train_df, valid_df, test_df, mtcnn):
        ...
        self.train_dataset = Ka2020DeepFack(train_df,  
            self.frame_sampler_para, face_defector_mctnn=mtcnn, 
            mctnn_size=self.mctnn_size, 
            outimg_size=self.outimg_size)

        self.vaild_dataset = Ka2020DeepFack(valid_df,  
            self.frame_sampler_para, face_defector_mctnn=mtcnn, 
            mctnn_size=self.mctnn_size, 
            outimg_size=self.outimg_size)

        self.test_dataset = Ka2020DeepFack(test_df,  
            self.frame_sampler_para, face_defector_mctnn=mtcnn, 
            mctnn_size=self.mctnn_size, 
            outimg_size=self.outimg_size)
    ...
    def sys_forward(self, i):
        pre_imgs, = i['frames'][:, 0, : , :, :, :] # batch, frames
        post_imgs = i['frames'][:, 1, : , :, :, :] 
        y_hat = self.model(  pre, post)
        return y_hat
    ...
    def get_logloss(self, y_hat, y):
        y = y.view(y.size(0), -1)
        y_hat = y_hat.view(y_hat.size(0), -1)
        loss = self.criterion_2(y_hat, y)
        return loss
